...
import (
	...
	"github.com/AnthonyHewins/aq/cmd/server/grpcserver"
	"github.com/AnthonyHewins/aq/cmd/server/rest"
...
func serveRESTProto() error {
	gin.SetMode(gin.ReleaseMode)

	serveAddr := fmt.Sprintf(":%d", *httpPort)
	httpServer = &http.Server{
		Addr:    serveAddr,
		Handler: rest.NewServer(logger, httpClient, dbReader, dbWriter, *noMiddlewares),
	}

	logger.Info("REST online", "listenaddr", serveAddr)
